...
| UIW053 | lista reordenável por arraste (kanban-lite) — o próximo passo do UIW005 | **17/09**: nasce `ReorderList(caption, labels, id)` sobre o MESMO mecanismo de ponteiro (eixo Y: `Event.y()`, o `reorderTarget`/`reorderAt` são puros, verde nos 3 alvos; live-snap com âncora fixa no mousedown como no Slider). ▲▼ por linha dão o caminho determinístico que roda nos 3 alvos; o arraste é real só no browser (prova: `scripts/browser-reorder.mjs` no Chrome — drag move a linha 0 duas posições e o clique ▲▼ commuta). **Nota:** montar este widget ACHOU o **Kof4j §266** (corpo de `view` com `if`+statements-depois no loop miscompilava no JS: os statements iam p/ a cláusula update do `for(;…;…)` reconstruído, `let` dos locais fora de escopo; em `view` o `catch` do `kofUiRender` engolia → UI VAZIA). §266 foi FECHADO no MESMO dia pela lane development (continue label estrutural `KofContinueLabel` + `JsLoopIfTailE2ETest` 7/7); o `ReorderList` voltou da função-auxiliar `reorderMark` p/ a forma `if` DIRETA no loop do view (prova viva: browser-reorder renderiza não-vazio + arrasta com a marca `(arrastando)`). As DUAS faces achadas ao montar este widget estão FECHADAS na plataforma (17/09): o §266 (corpo de loop com if+cauda) e o §267 (if/else de statement dobra em ternária e iça o `let` do statement seguinte — a face que o `ReorderList` NÃO toca, mas que o caso-G do harness do §266 media), com `JsLoopIfTailE2ETest` 7/7 + `JsIfFoldStatementE2ETest` 8/8 runBoth JVM+JS. Filha do §266 fechada na plataforma em 18/09: os catches do runtime UI (`kofUiRender`/effects/onMount/onDispose) que ENGOLIAM o throw agora REPORTAM `console.error("[kof] …: <stack>")` — crash de view não é mais UI vazia muda no browser (o flush continua resiliente: um component quebrado não derruba os irmãos). Prova: `ComponentCoreE2ETest.throwingViewIsReportedNotSilentlySwallowed`. Autor de widget não precisa mais de try/catch próprio p/ ver o erro. Prova: `tests/14-reorder.kf` (9 testes, valores exatos, trajetória desce→inverte→volta) + browser-reorder no DOM real |
